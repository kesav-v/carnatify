...
struct Args {
    /// File containing notes to parse
    #[arg(short, long, default_value = "")]
    input_file: String,

    /// String of notes to parse
    #[arg(default_value = "")]
    notes: String,

    /// Output file (.mid, .wav, .mp3, or .json). If omitted, plays audio.
    #[arg(short, long, default_value = "")]
    output_file: String,

    /// Pitch to play in (Sa is anchored to this note)
...
fn read_notes_from_file(file_name: &str) -> String {
...
fn main() {
    ...
    let notes = if !args.notes.is_empty() {
        ...
        read_notes_from_file(&args.input_file)
    };
